...
-- Q2 Use SELECT DISTINCT to select unique ProductName values from the Products table.
...
-- Q 4 Write a query to select all CustomerName values that start with 'A' using the LIKE operator.

select * from Customers
where FirstName like 'A%'

-- Q5 Order the results of a Products query by Price in ascending order
...
-- Q9 Use SELECT DISTINCT on two columns (Category and ProductName) in the Products table.

select distinct Category, ProductName from Products

-- Q 10 After SELECT DISTINCT on two columns (Category and ProductName) Order the results by ProductName in descending order.
...
alter table Sales
add constraint fk_prid foreign key (ProductID) references Products(ProductID)
...
Select * from Customers
Where Email like '%1@gmail.com' escape 1
